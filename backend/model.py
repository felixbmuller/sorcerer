...
class PadOfTruth:
    ...
    def __init__(self, players: List[Player]):
        ...
        self.actualTricks = [{p : 0 for p in self.players}]

    def lastRound(self) -> bool:
        """ TODO Check correct """
        return ((self.round == 9 and len(self.players) == 6)
            or (self.round == 11 and len(self.players) == 5)
            or (self.round == 14 and len(self.players) == 4)
            or (self.round == 19 and len(self.players) == 3))
    ...
    def announceTricks(self, player: Player, n: int):
        ...
        if player in self.announcedTricks[self.round]:
            raise StateError("A player cannot announce tricks twice for one round.")
        if n < 0:
            raise GameError("The number of announced tricks must be non-negative.")
    ...
    def finishRound(self):
        ...
        if not (len(self.announcedTricks[self.round]) == len(self.players) 
                and sum(self.actualTricks[self.round].values) == round + 1):
            raise StateError("Can only wrap up a round if every player announced tricks and all tricks were played")
        new_points = {}
        for p in self.players:
            diff = abs(self.announcedTricks[self.round][p] - self.actualTricks[self.round][p])
            if diff == 0:
...
class CardManager:
    ...
    def startRound(self, noOfCards: int):
        ...
        if self.roundStarted:
            raise StateError("Can only start a round if it is not already started!")
    ...
    def finishRound(self):
        if not self.roundFinished():
            ...
            raise StateError("Cannot finish round if not all cards were played")
        self.roundStarted = False

    def playCard(self, player: Player, card: str):
        if card not in self.handCards[player]:
            raise GameError(f"Player {player} tried to play {card} even though it is not in his hand!")
    ...
    def _checkPlayable(self, player: Player, card: str):
        ...
        if "Z" in self.roundCards.values()[0]: # first card wizard
            return # every card allowed
        if all("N" in c for c in self.roundCards.values()): # only jesters
            return # every card allowed
        relevantColor = next(c[0] for c in self.roundCards.values() if "Z" not in c and "N" not in c)
    ...
    def finishTrick(self) -> Player:
        ...
        if len(self.roundCards) != len(self.players):
            raise StateError("Tried to finish a trick before every player played a card")
    ...
    def _determineTrickTaker(self) -> Player:
        ...
        if any(self.trumpColor in c for c in self.roundCards.values()):
...
class Game:
    ...
    def __init__(self, players: List[Player]):
        ...
        self.trickStartPlayerIdx = 0 # player starting the trick, i.e. who won last trick or roundStartPlayerIdx
        self.currentPlayerIdx = 0

    def announceTricks(self, player: Player, n: int):
        if self.gameState != GameState.ANNOUNCE:
            raise GameError("Cannot announce tricks if not in announcement phase")
        ...
        if (self.currentPlayerIdx == (self.roundStartPlayerIdx - 1) % len(self.players)
            ...
            raise GameError("The number of announced tricks must not equal the number of actual tricks!") 
        ...
        if self.currentPlayerIdx == self.roundStartPlayerIdx:
            ...
            self.cardManager.startRound(self.padOfTruth.round + 1)
        ...
        if self.currentPlayerIdx == self.trickStartPlayerIdx:
            ...
            if self.cardManager.roundFinished():
                ...
                if self.padOfTruth.lastRound():
                    ...
                    self.gameState = GameState.ANNOUNCE
                    self.roundStartPlayerIdx = (self.roundStartPlayerIdx + 1) % len(self.players)
...
class Lobby:
    ...
    def addPlayer(self) :
        if len(self.players) >= 6:
            raise GameError("Cannot add another player, the maximum allowed number is 6")
        p = self._getFreeName()
    ...
    def startGame(self):
        ...
        if self.game is not None and self.game.gameState not in [GameState.ABORTED, GameState.FINISHED]:
            raise GameError("Game already running.")
        self.game = Game(self.players)

    def stopGame(self):
        if self.game is None or self.game.gameState not in [GameState.ANNOUNCE, GameState.PLAY]:
